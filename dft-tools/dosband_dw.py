...
with open(bandgap, "r") as file:
        lines = file.readlines()
        band_gap_down = lines[1].split()[5]  # Extract the value as a string
...
Bandat = './BAND/REFORMATTED_BAND_DW.dat'
datas_down = np.loadtxt(Bandat, dtype=np.float64)
...
# Plot DOS
axs[1].plot(total_down, energy_levels, label="Total DOS", color="xkcd:dark grey")
axs[1].fill_betweenx(energy_levels, total_down, color="xkcd:dark purple", alpha=0.5)
axs[1].plot(s_down, energy_levels, label="3s orbital", color="xkcd:brown")
axs[1].plot(p_down, energy_levels, label="3p orbital", color="xkcd:dark pink")
axs[1].grid(True, linestyle='-.')
axs[1].set_yticklabels([])  # Remove y-axis labels for band structure plot
axs[1].set_xlabel("Density of States")
axs[1].set_title(f"Density of States of $\\mathrm{{{element}}}₃$ (Spin DOWN)")
axs[1].set_xlabel("Density of States")
axs[1].legend()
axs[1].set_xlim(0, 5)
...
axs[0].plot(datas_down[:, 0], datas_down[:, 1:], linewidth=1.0, color='black')
axs[0].set_xticks(xtick)
axs[0].set_title(f"Band Structure for $\\mathrm{{{element}}}₃$ (Spin DOWN)")
...
axs[0].set_ylim(-3, 3)
axs[0].text(3.5, -2.65, f"Band Gap (eV): {band_gap_down}", fontsize=12, color='xkcd:dark red', ha='center', va='center')
plt.ylim((-3, 3))
...
plt.tight_layout()
png=f"DOSBAND_DW {element}3.png"
